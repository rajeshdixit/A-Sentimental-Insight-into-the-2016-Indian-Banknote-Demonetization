...
tweets <- list()
dates <- paste("2017-04-01",1:3,sep="") #SUGGESTION: Modify these dates of upto 7 days before running this program 
for (i in 2:length(dates)) {
  print(paste(dates[i-1], dates[i]))
  tweets <- c(tweets, searchTwitter("Demonetization", since=dates[i-1], until=dates[i], n=1000))
}
