...
Authentication <- OAuthFactory$new(consumerKey = consumerKey,
...
Authentication$handshake(cainfo ="G:/R/2016 Indian banknote demonetization/cacert.pem")
...
save(Authentication, file = "twitter authentication.Rdata")
...
demonetization_tweets = searchTwitter("#Demonetization", n=1000)

modi_tweets <- searchTwitter("#Modi", n=1000)

namo_tweets <- searchTwitter("#NaMo",n=10000)

class(demonetization_tweets)
length(demonetization_tweets)
head(demonetization_tweets)
userTimeline("rajeshdixit95")

install.packages("tm")
library("tm")

modi_list <- sapply(modi_tweets, function(x) x$getText())


#the function for extracting and analyzing tweets


search <- function(search_term, since, until)
{
  #extact tweets and create storage file
  list <- searchTwitter(search-trm, n=15, lang="en", since = )
  df <- twListToDF(list)
